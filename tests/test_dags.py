...
def test_antigravity_pipeline_structure(dagbag):
    ...
    expected_models = ["stg_telemetry", "stg_researchers", "dim_researchers", "fct_levitation_events"]
    
    for model in expected_models:
        assert any(model in task_id for task_id in task_ids), f"Model {model} not found in DAG tasks"
...
def test_quality_audit_structure(dagbag):
    ...
    assert "run_prod_tests" in task_ids, f"Expected task run_prod_tests not found. Tasks: {task_ids}"

@pytest.mark.skipif(os.getenv("CI") == "true", reason="dag.test() fails with serialization error in isolated CI")
